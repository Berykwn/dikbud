...
export default function TableCarousel({ carousels, allCarousels, pages }) {
    console.log(carousels)
    const { searchKeyword, handleSearchInputChange, filteredData } = useSearch(
        allCarousels,
        carousels,
        ["name"] //keyword pencarian
    );

    const handleDeleteConfirmation = () => {
        if (confirm("Apakah anda yakin akan menghapus data?") === false) {
            window.location.href(route("dashboard.carousels"));
        }
    };
                        ...
                        <tr key={item.id} className="bg-white">
                            ...
                            <td className="bg-white">
                                <div className="flex flex-wrap gap-2">
                                    <Linked
                                        href={route("dashboard.carousel.edit")}
                                        as="button"
                                        method="get"
                                        data={{ id: item.id }}
                                        variant={"dodger-blue"}
